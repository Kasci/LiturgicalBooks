...
for k in range(1,21):
    with io.open(f"1_generated/kaftizmy/K{k}.typ", "w", encoding="utf-8") as f:
        ...
        with io.open(f"../../../Liturgical_books_source/CSL/psaltyr/kaftizmy/K{k}.json", "r", encoding="utf-8") as inp:
            j = json.load(inp)
            generate(f,j,k)
...
with io.open(f"1_generated/0_all/Psaltyr.typ", "w", encoding="utf-8") as f:
    f.writelines([
        ...
        "\n",
        "#show: book\n\n"])
    with io.open(f"../../../Liturgical_books_source/CSL/psaltyr/general.json", "r", encoding="utf-8") as inp:
        j = json.load(inp)
