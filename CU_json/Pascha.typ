...
#let velkaEktenia() = {
  ...
  let mix = sk.zip(csl)
  let all = (
    table.cell(colspan: 3, inset: 1em, [== Ekténia]),
  )
  for (s, c) in mix {
...
#let piesen(idx) = {
  ...
  return (
    table.cell(colspan: 3, [=== Pieseň #idx]),
...
#let mala_ektenia(zvolanie_sk, zvolanie_csl) = {
  ...
  let last_csl = "Presvjatúju, prečístuju, preblahoslovénnuju, slávnuju Vladýčicu nášu Bohoródicu i prisnoďívu Maríju, so vsími svjatými pomjanúvše, sámi sebé i drúh drúha, i vés živót náš Christú Bóhu predadím."
  return (
    table.cell(colspan: 3, [=== Malá ekténia]),
    line,
...
#let kanon() = {
  ...
  return (
    table.cell(colspan: 3, inset: 1em, [== Kánon]),
    table.cell(colspan: 3, align: center, pText("Hlas " + str(sk.K.H))),
    piesen(1),
